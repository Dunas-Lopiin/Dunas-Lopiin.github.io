...
let tamanhoHomens = nomesMasculinos.length;
let tamanhoMulheres = nomesFemininos.length;
let tamanhoSobrenome = sobrenomes.length;
...
gravar.addEventListener("click", gravarNome);

function sortearNome(_array, _tipo, _quantidade){
    tamanhoSobrenome = sobrenomes.length;
    let sobrenomesFinal = "";
    ...
    for(let i = 0; i < _quantidade; i++){
        ...
        sobrenomesFinal = sobrenomesFinal + sobrenomes[linhaSobrenome] + " ";
    }
    return _array[nomeLinha] + " " + sobrenomesFinal;
}

function tipoDeSorteio(){
    tamanhoHomens = nomesMasculinos.length;
    tamanhoMulheres = nomesFemininos.length;
...
function novosNomes(_array, ...nomes){
    const novosNomes = [..._array, ...nomes];
    return novosNomes;
}

function gravarNome(){
    const tipoDeNome = document.getElementById("tipo-adicionar").value;
    let nomeAdicionar = document.getElementById("novo-nome").value;
    nomeAdicionar = nomeAdicionar.split(",");
    if(tipoDeNome === "masc"){
        nomesMasculinos = novosNomes(nomesMasculinos, ...nomeAdicionar);
        document.getElementById("novo-nome").value = "";
        console.log(nomesMasculinos);
    }
    else if(tipoDeNome === "sobre"){
        sobrenomes = novosNomes(sobrenomes, ...nomeAdicionar);
        document.getElementById("novo-nome").value = "";
        console.log(sobrenomes);
    }
    else{
        nomesFemininos = novosNomes(nomesFemininos, ...nomeAdicionar);
        document.getElementById("novo-nome").value = "";
